...
- **Thunderbolt.** A two-second attack with a “Pika… CHUUU!” caption. Real lightning, paced to the cry, aimed at your mouse pointer. The screen darkens a little and Pikachu's cheeks spark while it charges; on “CHUUU” a charge flashes up from its head into the sky, a leader races down from the top of the screen, and the return stroke blazes onto the pointer, a thin white channel with a blue-violet glow and a few dim branches, re-striking the same channel three more times while the whole screen blinks. The impact flashes, throws sparks and leaves a fading scorch glow; a smaller second strike lands on the tail of the cry, with a thunder crack and rumble and a slight shake. If the pointer is off that screen or right next to Pikachu, it strikes open ground on the far side instead. It's a click-through overlay, so it never gets in the way of your work. The focus reminder plays the pose without the screen flash.
...
- **Speech.** Whatever Pikachu says appears in a big yellow comic bubble above its head with outlined lettering: rounded for talk, a spiky electric burst for shouts. It pops in with a squash-and-stretch bounce, types the words out, wobbles, shrinks away, and scales with Pikachu's size.
